...
from visualization import visualize_routers_behaviors
...
def train(model, data_loader, optimizer, node_idx):
    ...
    if train_epoch_loss.cpu().numpy() < records['train_best_loss']:
...
def valid(model, data_loader, node_idx, struct):
    ...
    if valid_epoch_loss.cpu().numpy() < records['valid_best_loss']:
        records['valid_best_loss'] = valid_epoch_loss.cpu().numpy()

    if valid_epoch_accuracy.cpu().numpy() > records['valid_best_accuracy']:
    ...
    if not is_init_root_train and model.split and \
            valid_epoch_loss.cpu().numpy() < records['valid_best_loss_nodes_split'][node_idx]:
    ...
    if not is_init_root_train and model.extend and \
            valid_epoch_loss.cpu().numpy() < records['valid_best_loss_nodes_ext'][node_idx]:
    ...
    if is_init_root_train \
            and valid_epoch_loss.cpu().numpy() < records['valid_best_root_nosplit']:
    ...
    if not is_init_root_train and \
            valid_epoch_loss.cpu().numpy() < records['valid_best_loss_nodes'][node_idx]:
...
def checkpoint_model(model_file_name, struct=None, modules=None, model=None, figname='hist.png', data_loader=None):
    ...
    if args.visualize_split and not(data_loader is None):
        save_hist_dir = "./experiments/{}/{}/{}/{}".format(args.dataset, args.experiment, args.subexperiment, 'figures')
        visualize_routers_behaviors(model, data_loader, fig_scale=6, axis_font=20, subtitle_font=20, 
